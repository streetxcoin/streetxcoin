...
uint256 hashGenesisBlock("0x00000670b12f451cf0f2fa79a73e2a56bc3714d4ff6fe7fe3a201d761212dbcf");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // StreetXCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "StreetXCoin Signed Message:\n";
...
{
    // StreetXCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // StreetXCoin: each block
static const int64 nTargetSpacing = 3 * 60; // StreetXCoin: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // StreetXCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, StreetXCoin - DarkGravity, written by Evan Duffield - evan@StreetXCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // StreetXCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // StreetXCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "StreetXCoinX";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1501628637;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2677163;
        ...
        assert(block.hashMerkleRoot == uint256("0x0014a07745792e19fbdd3fc9168353697b793ac72e71d48277efe3c8f176cfe9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // StreetXCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("StreetXCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("StreetXCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static StreetXCoinMiner(CWallet *pwallet)
{
    printf("StreetXCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("StreetXCoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running StreetXCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StreetXCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StreetXCoinMiner, pwallet));
}
